...
# Determining fitness level ratios
fitnessLevel = [0.7, 1, 1.3]
level = 2
...
def generateConstraints(lines):
    ...
    for nut in nutritionRange:
        ...
        constraintValues += " >= " + str(float(nutrition[nut][1]) * fitnessLevel[level]) + "; \n"
        ...
        for places in restaurants:
            for foods in places:
                for times in range(0, 3):
                    constraintValues += "+" + str(lines[foods][nutrition.index(nutrition[nut])]) + \
                                        " y_" + str(times) + "_" + str(foods) + " "
        constraintValues += " <= " + str(float(nutrition[nut][2]) * fitnessLevel[level]) + "; \n"
